 ...
 * Copyright (C) 2014 Cryptotronix, LLC. Debugged, Sanitized, Extended by Riddle&Code 2015
 *
...
public:
  ...
  uint8_t wakeup();
  uint8_t resync(uint8_t rx_size, uint8_t *rx_buffer);
  ...
  uint8_t derEccKey(const uint8_t KEY_ID);
  int getNonce(uint8_t *to_load, int len);
