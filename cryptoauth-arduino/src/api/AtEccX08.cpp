 ...
 * Cryptotronix, LLC. Debugged, Sanitized, Extended by Riddle&Code 2015
 *
...
uint8_t AtEccX08::resync(uint8_t size, uint8_t *response)
{
  //return eccX08c_resync(rx_size, rx_buffer);
...
uint8_t AtEccX08::getRandom(bool update_seed)
{
    ...
    //RANDOM_MODE_NO_SEED_UPDATE);
    uint8_t SEED_UPDATE;

    if (update_seed)
      SEED_UPDATE = RANDOM_MODE_SEED_UPDATE;
    else
      SEED_UPDATE = RANDOM_MODE_NO_SEED_UPDATE;
...
//  uint8_t config_data[ECCX08_CONFIG_SIZE];
  ...
  ret_code = eccX08m_execute(ECCX08_LOCK,
                             LOCK_MODE_CONFIG | LOCK_MODE_NO_CRC, 0,
                             0, NULL, 0, NULL, 0, NULL,
                             sizeof(this->command), this->command,
                             sizeof(this->temp), this->temp);
...
//  uint8_t config_data[ECCX08_CONFIG_SIZE];
  ...
  ret_code = eccX08m_execute(ECCX08_LOCK,
                             ...
                             ((slotNum & 0x0f) << 2) | LOCK_MODE_NO_CRC, 0,
                             0, NULL, 0, NULL, 0, NULL,
                             sizeof(this->command), this->command,
                             sizeof(this->temp), this->temp);
...
int AtEccX08::getNonce(uint8_t *to_load, int len)
{
  // Pass the message to be signed using Nonce command with mode =
  // 0x03.
  uint8_t *rsp_ptr = &this->temp[ECCX08_BUFFER_POS_DATA];
 ...
 *
 */
uint8_t AtEccX08::derEccKey(const uint8_t KEY_ID)
{
  uint8_t *rsp_ptr = &this->temp[ECCX08_BUFFER_POS_DATA];
  ...
  int ret_code =
    eccX08m_execute(ECCX08_DERIVE_KEY, NONCE_MODE_PASSTHROUGH,
